"""
マピオン電話帳 「興信所・探偵事務所」 (mapion_2) — 関東甲信越東海 14 都県スクレイパー

対象サイト: https://www.mapion.co.jp/phonebook/M10037/13/

既存サイト `mapion` (site_id=mapion, scripts/sites/service/mapion.py / 大阪・兵庫・京都の
3 府県版) と同一カテゴリ (M10037) ・同一サイト構造の別都県版。取得ロジックは共通で、
対象都道府県セット (_TARGET_PREFS) だけが異なる。
...
利用規約の確認結果 (重要):
    ...
    → 既存サイト mapion と同じ判断で、生成・実行を継続する。

取得対象:
    - マピオン電話帳のカテゴリ「興信所・探偵事務所」(M10037) に掲載されている
      関東甲信越東海 14 都県の事業者
      東京都(13) 神奈川県(14) 埼玉県(11) 千葉県(12) 茨城県(08) 栃木県(09) 群馬県(10)
      新潟県(15) 山梨県(19) 長野県(20) 岐阜県(21) 静岡県(22) 愛知県(23) 三重県(24)
    - 取得カラムは 名称 / TEL / 住所 / 都道府県 の 4 列 (備考の指示どおり)

サイト構造 (live 実機で確認済み):
    - 府県ページ …/M10037/{2桁}/
        * 東京都(13, 33 リンク) / 神奈川県(14) / 埼玉県(11) / 茨城県(08) など掲載数の多い県:
          掲載のある市区町村ページへのリンクのみ (一覧テーブル無し)
        * 山梨県(19) / 三重県(24) など掲載数の少ない県:
          市区町村リンクが無く、府県ページ自体に一覧テーブル (table.list-table) がある
        → 両方に対応するため「府県ページに一覧テーブルがあれば府県ページも巡回対象」とする
    - 市区町村ページ …/M10037/{5桁}/ に一覧テーブル (table.list-table)
        ...
        * 政令市 (例: 横浜市 14100 / 名古屋市 23100) の一覧には
          行政区コード配下の詳細 URL が混在する
    ...
    - 詳細ページ …/phonebook/M{カテゴリ}/{5桁}/{spot_id}/
        ...
        * 住所は詳細ページにのみ掲載 (「〒105-0004」+ 住所)
        * 詳細 3 件 (東京都港区 / 山梨県甲府市 / 三重県四日市市) をサンプリングし
          3/3 で同一構造であることを確認済み

取得フロー:
    1. 引数 url からカテゴリのルート (…/phonebook/M10037/) を導出し、
       対象 14 都県の入口ページ (…/M10037/{13,14,11,…}/) を組み立てる。
       ※ 先頭は引数 url の都県コードそのもの。別ルート URL はハードコードしない。
    2. 府県ページから一覧ページ (市区町村ページ) を列挙する。
       府県ページ自体に一覧テーブルがあれば府県ページも巡回対象に含める (山梨・三重対応)。
...
注意:
    ...
    - 都道府県が対象 14 都県以外の事業者は除外する (備考の指示)。
    - 詳細ページが取得できなかった場合は一覧の 店舗名 / TEL と
      都県コード由来の都道府県だけで出力し (住所は空文字)、取りこぼしを防ぐ。
    - よみがな / 最寄り駅 / 標高 / マップコード等は備考の 4 列指定外のため取得しない。
      説明文などの自由記述カラムも取得しない (著作権リスク回避)。

実行方法:
    # ローカルテスト
    python scripts/sites/service/mapion_2.py

    # Prefect Flow 経由
    docker compose exec worker python /app/bin/run_flow.py --site-id mapion_2
"""
...
# 対象 14 都県 (JIS コード → 都道府県名)。備考「14 都県以外は対象外」に対応。
# 先頭は引数 url の都県コード (13 = 東京都) に合わせている。
_TARGET_PREFS = {
    "13": "東京都",
    "14": "神奈川県",
    "11": "埼玉県",
...
# ※ 長い名称を先に並べる必要はない (完全な都道府県名のみを列挙しているため曖昧性は無い)
...
# 一覧のページ送りリンク (…/13103/2.html) からページ番号を取り出す
...
class Mapion2(StaticCrawler):
    """マピオン電話帳 興信所・探偵事務所 (関東甲信越東海 14 都県) スクレイパー"""
    ...
    def _category_root(url: str) -> str:
        """引数 url (…/phonebook/M10037/13/) からカテゴリルート (…/phonebook/M10037/) を導出する。"""
        base = url if url.endswith("/") else url + "/"
        # 末尾が都県コード等の数字ディレクトリなら 1 階層上がカテゴリルート
    ...
    def _collect_list_urls(self, pref_url: str, cat_path: str) -> list[str]:
        ...
        山梨県・三重県のように市区町村リンクを持たず府県ページ自体が一覧になっている
    ...
    def _build_item(self, detail_url: str, list_name: str, list_tel: str, pref_name: str) -> dict | None:
        ...
        # 住所: 「〒105-0004 東京都港区新橋６丁目５−４」→ 都道府県と市区町村以降に分解
        ...
        # 都道府県が読めない場合は巡回中の都県コード由来の名称で補完する
        if not pref:
            pref = pref_name
...
if __name__ == "__main__":
    ...
    scraper.execute("https://www.mapion.co.jp/phonebook/M10037/13/")
