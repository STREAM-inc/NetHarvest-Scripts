"""
ヒカカク！【出張買取業者】(hikakaku.com) スクレイパー

取得対象:
    https://hikakaku.com/company/kaitoriinfo/field/keishiki/syuccho/
    出張買取 (訪問買取) に対応する買取業者の一覧 (全国分・都道府県絞り込み無し)

取得フロー:
    1. ルート URL をページネーション (?page=N) で巡回。1 ページ 20 件。
       カードが 0 件になったページで終了する。
       2026-08 時点の実測: 最終ページ = 322 (11 件)、合計約 6,431 件。
    2. 一覧カード (section.coSummaryStore-Wrapper) から
       業者名 / 掲載ページURL / 業者ID / 買取形式 / 対応地域 / 古物商許可番号 / 住所 /
       送料 / 電話番号 (PR 掲載枠のみ) / 評価スコア / クチコミ件数 / 査定実績件数 を抽出
    3. 業者詳細ページ (/company/{id}/) を 1 件ずつ取得し、
       電話番号 (CV枠) / 運営会社 / 運営会社の住所 / 屋号の運営代表者名 / 本店の住所 /
       本店の営業時間 / 本店の定休日 / 法人買取対応 / LINE査定 / 梱包材 / 振込手数料 /
       取扱い商品 / 公式サイト / X アカウント / 店舗一覧URL を補完
...
構造上の注意点:
    ...
    - 詳細ページのラベルは <br> で改行される (例: 「現金化<br>スピード」) ため、
      照合前に空白を全除去して正規化する。
    - 一覧の住所は「神奈川県神奈川県横浜市…」のように都道府県が重複することがあるため
      正規化してから PREF / ADDR に分割する。
    - 電話番号は PR 掲載枠の業者にのみ表示される。詳細ページの CV ボタン横
      (div.store-cv-tel-number) が主ソースで、一覧カードの tel: リンクを予備とする。
      実測では 1 ページ 20 件中 3 件のみ。掲載が無い業者は空文字となる。
    - 「未記載」はサイトの空値プレースホルダなので空文字に変換する。
    - 郵便番号 / 法人番号 / 資本金 / 売上 / 従業員数 / 設立日 / FAX / メールアドレス /
      Instagram / Facebook / LINE 公式アカウントはサイト上に項目自体が存在しないため
      取得できない (常に空欄)。
    - 店舗説明文 (coSummaryStore-Description)・キャッチコピー・特記事項・
      キャンセルポリシー・現金化スピード・コロナウイルス対策・よくある質問と回答・
      状態別の買取価格・口コミ本文は自由記述の文章のため、著作権リスクを避けて取得しない。

実行方法:
    python scripts/sites/service/hikakaku_4.py
    python bin/run_flow.py --site-id hikakaku_4
"""
...
class Hikakaku4Scraper(StaticCrawler):
    """ヒカカク！【出張買取業者】スクレイパー"""
    ...
    _LIST_LABELS = {
        ...
        "送料": "shipping",
        "梱包材": "packing",
        "買取代金振込手数料": "transfer_fee",
        "振り込み手数料": "transfer_fee",
    }
    ...
    def _parse_card(self, card, page_url: str) -> dict | None:
        ...
        return {
            ...
            "法人買取対応": "",
            "LINE査定": "",
            "送料": values.get("shipping", ""),
            "梱包材": values.get("packing", ""),
            "買取代金振込手数料": values.get("transfer_fee", ""),
    ...
    def _enrich_from_detail(self, item: dict) -> None:
        ...
        # 一覧に無い/空の項目は詳細側で補完する
        for extra_key, value_key in (
            ("古物商許可番号", "kobutsu"),
            ("送料", "shipping"),
            ("梱包材", "packing"),
            ("買取代金振込手数料", "transfer_fee"),
        ):
            if not item[extra_key]:
                item[extra_key] = values.get(value_key, "")
        ...
        # 電話番号: CV ボタン横の番号 (PR 掲載業者のみ)。一覧の PR 番号より優先する。
        tel_node = soup.select_one(".store-cv-tel-number")
        tel = _clean(self._text_of(tel_node))
        if tel:
            item[Schema.TEL] = tel

        # 取扱い商品: div.maps_list 内の「〜の取扱い商品」見出しに続くタグリスト
        # (サイトが定義した買取ジャンルそのものなので CAT_SITE にも入れる)
        categories = self._extract_categories(soup)
        item["取扱い商品"] = categories
        item[Schema.CAT_SITE] = categories

        # 店舗一覧ページ (支店を持つ業者のみ)
        shops_link = soup.select_one(".companyShow-ShopsLink a[href]")
        if shops_link:
            item["店舗一覧URL"] = urljoin(detail_url, shops_link["href"])

        # 公式サイト / X アカウント
        official = soup.select_one("a.companyShow-InfoContainer_ExternalLink[href]")
        if official:
            item[Schema.HP] = official["href"]
        twitter = soup.select_one("a.companyShow-InfoContainer_TwitterIconLink[href]")
        if twitter:
            item[Schema.X] = twitter["href"]
    ...
    @staticmethod
    def _extract_categories(soup) -> str:
        """「{業者名}の取扱い商品」ブロックの品目タグを / 区切りで返す。"""
        for block in soup.select("div.maps_list"):
            heading = block.select_one("h2, h3")
            if heading is None or "取扱い商品" not in heading.get_text():
                continue
            names = [_clean(a.get_text()) for a in block.select(".tag_list a")]
