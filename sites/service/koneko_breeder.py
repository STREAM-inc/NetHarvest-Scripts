"""
みんなのブリーダー（猫） — 猫ブリーダー検索ポータル

取得対象:
    - 全国の猫ブリーダー一覧（事業所名・所在地・代表者・動物取扱業登録情報・評価等）

取得フロー:
    一覧ページ (breederList.php, pageNum=N でページ送り, 1ページ10件) を巡回し、
    各ブリーダーの詳細ページ (breeder_xxxx.html) を取得して即 yield する (Pattern B)。
    - 一覧: 名称・総合評価・口コミ件数・子猫掲載数
    - 詳細: 事業所名・所在地(住所)・種別・氏名(代表者)・動物取扱責任者・登録番号 等
            名称カナは <title> の括弧内から抽出

実行方法:
    # ローカルテスト
    python scripts/sites/service/koneko_breeder.py

    # Prefect Flow 経由
    docker compose exec worker python /app/bin/run_flow.py --site-id koneko_breeder
"""
...
class KonekoBreeder(StaticCrawler):
    """みんなのブリーダー（猫） スクレイパー"""
        ...
        while True:
            ...
            for box in boxes:
                try:
                    ...
                    if grid:
                        m = re.search(r"子猫一覧\s*([\d,]+)件", grid.get_text(" ", strip=True))
                        if m:
                            kitten = m.group(1).replace(",", "")
    ...
    def _scrape_detail(self, url: str) -> dict | None:
        ...
        # 名称カナ: <title>【神奈川/猫】朝比奈　杏奈(あさひな　あんな)ブリーダー｜... の括弧内
        if soup.title:
...
if __name__ == "__main__":
    ...
    scraper = KonekoBreeder()
    # 🔒 sites.yml に登録する url と完全一致させること (SSOT = sites.yml)。
    scraper.execute("https://www.koneko-breeder.com/breederList.php?key_b_name=")
