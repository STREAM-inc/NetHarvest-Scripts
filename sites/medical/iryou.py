...
取得対象:
    ...
    - 診療科目別診療時間（動的生成）:
        {科目名}_月〜日（7カラム）: 診療時間を "08:30-12:00,14:00-18:00" 形式で格納
        {科目名}_初診予約 / 予約外診察 / 入院受入 / 女性医師（4カラム）
...
class IryouScraper(StaticCrawler):
    ...
    def _dept_hours(self, soup) -> dict:
        ...
        カラム名:
            {科目名}_月〜日  : "08:30-12:00,14:00-18:00" / "-"
            {科目名}_初診予約 / 予約外診察 / 入院受入 / 女性医師 : "実施" / "可能" / "不可" / None
        """
        result: dict = {}
        ...
        for strong in soup.find_all("strong"):
            ...
            for day in _DAYS:
                slots = day_slots[day]
                result[f"{dept_name}_{day}"] = ",".join(slots) if slots else "-"
            for attr_key, attr_val in attrs.items():
                result[f"{dept_name}_{attr_key}"] = attr_val
