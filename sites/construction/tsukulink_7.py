"""
ツクリンク（関東） (tsukulink.net) — 関東エリアの建設業者スクレイパー

取得対象:
    備考「関東エリアのみ」に従い、関東 1 都 6 県
    (茨城県・栃木県・群馬県・埼玉県・千葉県・東京都・神奈川県) を
    拠点とする建設業者のみを取得する。
    2026-09 時点 東京 75,244 / 神奈川 47,115 / 埼玉 41,145 / 千葉 31,883 /
    茨城 19,056 / 群馬 12,610 / 栃木 11,913 = 約 238,966 社 (20 件/ページ)。

取得フロー:
    1. 引数 url (トップページ) を取得し、ページ内の都道府県ナビから
       関東 7 県の県別一覧 URL (/tokyo /kanagawa …) を導出する
       (リンクが取れない場合は url を基準にスラッグから組み立てる)
    2. 7 県の一覧 {pref_url}?page=N をラウンドロビンで 1 ページ (20 件) ずつ取得
       (東京だけで 3,700 ページ超あるため、県を順に回して関東全体へ分散させる)
    3. li.p-companies-list-item から会社ID・詳細URL・業種・主力工事・工事区分を拾う
    4. 詳細ページを 1 件取得するごとに即 yield (Pattern B)
    5. 会社IDで重複排除し、詳細ページの住所が関東 7 都県でないものは除外する

サイト仕様メモ:
    - 一覧・詳細とも Accept ヘッダが無いと HTTP 400 を返すため _setup() で付与する
    - 全国一覧 /companies に都道府県の絞り込みクエリは存在しない
      (?prefecture= 等はすべて無視される)。県で絞るにはトップページの
      都道府県ナビにある県別一覧 /tokyo /kanagawa … を辿るしかない
    ...
    - 利用規約 (/term) にスクレイピング・クローリングの禁止条項は無く、
      robots.txt も県別一覧・会社詳細を許可している (2026-09 時点)
...
実行方法:
    # ローカルテスト
    python scripts/sites/construction/tsukulink_7.py

    # Prefect Flow 経由
    docker compose exec worker python /app/bin/run_flow.py --site-id tsukulink_7
"""
...
# 備考「関東エリアのみ」で指定された対象都道府県。
# key = 正式名 (詳細ページの住所・パンくずと照合する)
# value = (URL スラッグ, トップページの都道府県ナビ上の表記)
_KANTO_PREFS = {
    "東京都": ("tokyo", "東京"),
    "神奈川県": ("kanagawa", "神奈川"),
    "埼玉県": ("saitama", "埼玉"),
...
# ナビ表記 (「東京」) → 正式名 (「東京都」) の逆引き
_NAV_LABEL_TO_PREF = {nav: pref for pref, (_slug, nav) in _KANTO_PREFS.items()}

# 県別一覧のパス (/tokyo 等) の照合用
_PREF_LIST_PATHS = {f"/{slug}": pref for pref, (slug, _nav) in _KANTO_PREFS.items()}
...
# 1 県あたりのページ送り安全上限
# (2026-09 時点で最多の東京都が 75,244 件 = 3,763 ページ)
_MAX_PAGES = 10000


class TsukulinkKanto(StaticCrawler):
    """ツクリンク（関東） — 関東 1 都 6 県の建設業者スクレイパー"""
    ...
    def parse(self, url: str):
        """引数 url (トップページ) を唯一の起点に、関東 7 都県の一覧→詳細を巡回する。"""
        pref_lists = self._kanto_list_urls(url)
        ...
        while pages:
            for pref in list(pref_lists):
                ...
                soup = self.get_soup(f"{pref_lists[pref]}?page={page}")
                if soup is None:
                ...
                if page == 1:
                    # 「N件中」表示から関東 7 県分の総件数を積み上げて ETA を有効にする
                ...
                for li in items:
                    ...
                    if not item:
                        continue
                    # 備考「関東エリアのみ」: 詳細ページの住所が関東 7 都県のものだけ採用する
                    if item[Schema.PREF] not in _KANTO_PREFS:
                        logger.debug("関東外のため除外: %s (%s)", detail_url, item[Schema.PREF])
                        continue
                    yield item
    ...
    def _kanto_list_urls(self, url: str) -> dict:
        """起点 (トップページ) の都道府県ナビから関東 7 都県の一覧 URL を導出する。

        戻り値は {"東京都": "https://tsukulink.net/tokyo", ...}。
        ナビからリンクを拾えなかった県は url を基準にスラッグから組み立てる。
        """
        found: dict[str, str] = {}
        ...
        if soup is not None:
            for a in soup.select("a[href]"):
                ...
                pref = _PREF_LIST_PATHS.get(path)
                if not pref:
                    continue
                # ナビ表記 (「東京」) と一致するリンクだけを一覧起点として採用する
                label = _clean(a.get_text(" ", strip=True))
                if label and _NAV_LABEL_TO_PREF.get(label) != pref:
                    continue
                found.setdefault(pref, candidate)

        for pref, (slug, _nav) in _KANTO_PREFS.items():
            if pref not in found:
                logger.warning("都道府県ナビから %s の一覧リンクを検出できないため補完する", pref)
                found[pref] = urljoin(url, f"/{slug}")

        # 掲載社数の多い順 (東京→神奈川→…) ではなく定義順に回す
        return {pref: found[pref] for pref in _KANTO_PREFS if pref in found}
