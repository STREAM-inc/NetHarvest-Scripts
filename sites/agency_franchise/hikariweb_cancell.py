...
BASE_URL = "https://hikariweb.ntt-east.co.jp/general/cancell"
STATUS = "解約特約店・販売委託店"
...
class HikaridwebCancellScraper(StaticCrawler):
    """NTT東日本解約特約店・販売委託店検索スクレイパー

    調査フェーズ: あ行〜わ行(keyword=1-10)の全一覧ページから詳細URLを収集。
    実行フェーズ: 各詳細ページ(解約申請受理日・解約後保守会社・保守連絡先)をスクレイピング。
    """

    DELAY = 1.5
    EXTRA_COLUMNS = ["解約申請受理日", "解約後保守会社", "保守連絡先", "委託元", "販売区分", "備考"]
    ...
    def _scrape_detail(self, url: str) -> dict | None:
        """Phase 2: 詳細ページから全フィールドを取得（解約固有フィールド含む）"""
...
if __name__ == "__main__":
    ...
    HikaridwebCancellScraper().execute(f"{BASE_URL}/list.php?keyword=1")
