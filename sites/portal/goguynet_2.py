...
goguynet_2 と既存 goguynet の違い（このクローラーの主眼）:
    既存の `goguynet` はトップページの全国横断フィード (?page=N) を巡回するだけで、
    各市区町村サイト固有のアーカイブには入っていかない。実測すると号外NET の
    市区町村サイト (例: sapporochuo.goguynet.jp) は `?page=N` を無視して常に
    1 ページ目を返すため、旧方式では各地域サイトの過去記事を取り逃していた。

    goguynet_2 は「全国各地の地域サイトを個別に巡回する」ことを主眼とする:
      1. トップ (https://goguynet.jp/) のナビから全国の
         「市区町村サブドメイン」(例: https://bunkyo.goguynet.jp/) と
         「都道府県パス」(例: https://goguynet.jp/hokkaido/) を列挙する。
      2. 各地域サイトを WordPress のパス形式ページ送り `/page/N/` で巡回する。
         この形式はトップ・都道府県パス・市区町村サブドメインの全てで機能し、
         末尾超過で 404 (＝取得終了) になるため確実に全ページを辿れる。
         （`?page=N` はサブドメインで無視されるため使わない。）

取得対象（飲食店のみ）:
    各地域サイトの最新記事フィード (a.itemTitle01) のうち、飲食店
    （レストラン・カフェ・居酒屋・ラーメン・パン/スイーツ店 等）に該当する
    記事のみ。各記事詳細ページの .shop-info（店舗名・住所・営業時間・定休日・
    最寄り駅・電話番号・関連リンク）を取得する。
...
取得フロー:
    トップ（＝引数 url）→ 地域サイト列挙
      → 各地域サイトを /page/N/ で巡回
      → 一覧 (a.itemTitle01) から飲食店判定を通過したものだけ
      → 詳細ページ (.shop-info の dt/dd) を1件ずつ取得して即 yield (Pattern B)
      → 詳細URLで全地域横断の重複排除（トップと地域サイトは記事が重複するため）
...
実行方法:
    python scripts/sites/portal/goguynet_2.py
    docker compose exec worker python /app/bin/run_flow.py --site-id goguynet_2
"""
...
# 都道府県パス判定用の 47 都道府県ローマ字（トップのナビから都道府県パスを列挙する）
_PREF_ROMAJI = {
...
# ナビに現れる goguynet.jp サブドメイン (市区町村サイト) を拾う
_SUBDOMAIN_RE = re.compile(r"^https?://([a-z0-9\-]+)\.goguynet\.jp", re.I)
# 都道府県パス https://goguynet.jp/hokkaido/ を拾う
_PREFPATH_RE = re.compile(r"^https?://goguynet\.jp/([a-z]+)/?(?:$|[?#])", re.I)

# 1 地域サイトあたりのページ送り安全上限（末尾超過は 404 で終了するが保険として設定）
_MAX_PAGES_PER_REGION = 500


class Goguynet2Scraper(StaticCrawler):
    """号外NET 飲食店2 (goguynet.jp) 全国地域サイト巡回スクレイパー"""

    DELAY = 1.5
    EXTRA_COLUMNS = ["掲載日時", "地域", "最寄り駅"]

    def parse(self, url: str) -> Generator[dict, None, None]:
        seen: set[str] = set()  # 詳細URL（地域横断の重複排除）

        # トップページを取得し、全国の地域サイト（サブドメイン＋都道府県パス）を列挙する。
        root_soup = self.get_soup(url)
        if root_soup is None:
            return
        regions = self._discover_regions(url, root_soup)
        self.logger.info("巡回対象の地域サイト数: %d", len(regions))

        # 最初の地域（＝トップ url 自身）から順に、各地域サイトを /page/N/ で巡回する。
        # トップは全国横断フィードなので早い段階で飲食店を yield でき、テスト実行の
        # 「最初の1件を数秒以内に yield」要件も満たす。
        for base in regions:
            yield from self._crawl_region(base, url, seen)

    def _discover_regions(self, url: str, soup) -> list[str]:
        """トップページのリンクから巡回対象の地域サイト起点URLを列挙する。

        戻り値は [引数 url] を先頭に、市区町村サブドメイン・都道府県パスを続けた
        重複無しの順序付きリスト。引数 url を唯一のルートとして起点に含める。
        """
        ordered: list[str] = []
        ...
        # 引数 url（トップ）を最初の地域として必ず含める
        add(url)

        root_host = urlparse(url).netloc.lower()
        for a in soup.find_all("a", href=True):
            href = a["href"].strip()
            m = _SUBDOMAIN_RE.match(href)
            if m and m.group(1).lower() not in ("www",):
                host = m.group(1).lower()
                # goguynet.jp 本体ホスト（www 無し）はサブドメイン扱いしない
                if f"{host}.goguynet.jp" != root_host:
                    add(f"https://{host}.goguynet.jp/")
                continue
            m2 = _PREFPATH_RE.match(href)
            if m2 and m2.group(1).lower() in _PREF_ROMAJI:
                add(f"https://goguynet.jp/{m2.group(1).lower()}/")

        return ordered

    def _crawl_region(
        self, base: str, root_url: str, seen: set
    ) -> Generator[dict, None, None]:
        """1 つの地域サイトを WordPress パス形式 /page/N/ で全ページ巡回する。"""
        prev_sig = None
        for page in range(1, _MAX_PAGES_PER_REGION + 1):
            # ページ送りは /page/N/ に統一（?page=N はサブドメインで無視されるため）。
            # 1 ページ目は base そのもの。
            page_url = base if page == 1 else urljoin(base, f"page/{page}/")
            soup = self.get_soup(page_url)
            if soup is None:  # 末尾超過（404 等）→ この地域は終了
                break
            anchors = soup.select("a.itemTitle01")
            if not anchors:
                break

            # 同一シグネチャの再取得（ページ送りが効かず先頭ページを返し続ける状態）を検知して打ち切る
            sig = tuple(a.get("href") for a in anchors)
            if sig == prev_sig:
                break
            prev_sig = sig
            ...
            for a in anchors:
                ...
                if detail_url in seen:  # 地域横断の重複記事はスキップ
                    continue
                seen.add(detail_url)
                ...
                try:
                    item = self._scrape_detail(
                        detail_url, list_title, category, post_date
                    )
    ...
    def _name_from_title(title: str) -> str:
        ...
        文章丸ごとが名称になってしまう。以下の順で固有名だけを取り出す。

        1. 「」『』"" で囲まれた固有名があれば最優先で採用する。
        2. 角括弧の地域プレフィックス（【茨木市】等）を除去する。
        3. 先頭の煽り導入節と日付・期間トークン列を剥がす。
        4. 最初の節（、。！の手前まで）を取り出し、末尾の述語を落とす。
        5. 先頭の場所プレフィックス「◯◯で」を除去する。
    ...
    def _is_restaurant(text: str) -> bool:
        ...
        （_EVENT_WORDS）を含まないものを常設の飲食店とみなす。号外NET には飲食
        ジャンルの構造化データが無いため、この文字列判定が唯一の分類手段となる。
        """
        if not text:
            return False
