"""
人材サービス総合サイト — 労働者派遣事業者スクレイパー（合同会社）

取得対象:
    人材サービス総合サイトの開業届出・事業所一覧検索から
    全国 × 事業主名称「合同会社」部分一致 × 区分「労働者派遣事業」
    に絞り込んだ全事業所の詳細情報

取得フロー（2フェーズ並列化）:
    Phase1: 「合同会社」全ページを逐次POSTしてURLを収集・重複排除
    Phase2: 詳細ページを MAX_WORKERS 並列で GET

実行方法:
    python scripts/sites/portal/jinzai_service_rodosha_haken_godokaisha.py

    Prefect Flow 経由:
    python bin/run_flow.py --site-id jinzai_service_rodosha_haken_godokaisha
"""
...
SITE_NAME = "人材サービス総合サイト_労働者派遣事業_合同会社"
...
class JinzaiServiceRodoshaHakenGodokaishaScaper(StaticCrawler):
    """人材サービス総合サイト 労働者派遣事業者スクレイパー（合同会社 × 並列取得）"""
...
if __name__ == "__main__":
    ...
    scraper = JinzaiServiceRodoshaHakenGodokaishaScaper()
