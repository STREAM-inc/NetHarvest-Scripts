"""
求人ボックス（農業・林業・畜産・酪農）スクレイパー

取得対象:
    農業 / 林業 / 畜産 / 酪農 キーワード検索結果（全国）

取得フロー:
    /adv/ キーワード検索URL → 一覧カード（data-func-show-arg JSON）+ 内部詳細ページ補完

実行方法:
    # ローカルテスト
    python scripts/sites/jobs/kyujin_box_norin.py

    # Prefect Flow 経由
    python bin/run_flow.py --site-id kyujin_box_norin
"""
...
LIST_URL = (
    ...
    "?keyword=%E8%BE%B2%E6%A5%AD%20or:%E6%9E%97%E6%A5%AD%20or:%E7%95%9C%E7%94%A3%20or:%E9%85%AA%E8%BE%B2"
    "&area="
)
...
class KyujinBoxNorinScraper(StaticCrawler):
    """求人ボックス（農業・林業・畜産・酪農）スクレイパー"""
