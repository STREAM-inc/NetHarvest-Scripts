...
class JobconDriverScraper(DynamicCrawler):
    """ジョブコンプラスD ドライバー求人スクレイパー（job-con.jp/driver）"""
    ...
    def _collect_detail_urls(self, start_url: str) -> list[str]:
        ...
        while current:
            ...
            for a in soup.select("a.job-card__element.--title, a[href*='/driver/detail/']"):
                href = a.get("href", "").strip()
                if not href:
                    # try building from data attributes
                    company_id = a.get("data-company-id", "")
                    offer_id = a.get("data-offer-id", "")
                    if company_id and offer_id:
                        href = f"/driver/detail/{company_id}/{offer_id}"
...
if __name__ == "__main__":
    ...
    JobconDriverScraper().execute("https://job-con.jp/driver/search")
