"""
求人ボックス（食品製造の仕事）スクレイパー

取得対象:
    - 食品製造の仕事

取得フロー:
    一覧カード（data-func-show-arg JSON）+ 内部詳細ページ補完

実行方法:
    # ローカルテスト
    python scripts/sites/jobs/kyujin_box_shokuhin_seizou.py

    # Prefect Flow 経由
    python bin/run_flow.py --site-id kyujin_box_shokuhin_seizou
"""
...
LIST_URL = "https://xn--pckua2a7gp15o89zb.com/%E9%A3%9F%E5%93%81%E8%A3%BD%E9%80%A0%E3%81%AE%E4%BB%95%E4%BA%8B"

# 一都三県の絞り込みURL
_PREF_URLS = [
    "https://xn--pckua2a7gp15o89zb.com/%E9%A3%9F%E5%93%81%E8%A3%BD%E9%80%A0%E3%81%AE%E4%BB%95%E4%BA%8B-%E6%9D%B1%E4%BA%AC%E9%83%BD",       # 東京都
    "https://xn--pckua2a7gp15o89zb.com/%E9%A3%9F%E5%93%81%E8%A3%BD%E9%80%A0%E3%81%AE%E4%BB%95%E4%BA%8B-%E7%A5%9E%E5%A5%88%E5%B7%9D%E7%9C%8C",  # 神奈川県
    "https://xn--pckua2a7gp15o89zb.com/%E9%A3%9F%E5%93%81%E8%A3%BD%E9%80%A0%E3%81%AE%E4%BB%95%E4%BA%8B-%E5%9F%BC%E7%8E%89%E7%9C%8C",       # 埼玉県
    "https://xn--pckua2a7gp15o89zb.com/%E9%A3%9F%E5%93%81%E8%A3%BD%E9%80%A0%E3%81%AE%E4%BB%95%E4%BA%8B-%E5%8D%83%E8%91%89%E7%9C%8C",       # 千葉県
]
...
class KyujinBoxShokuhinSeizouScraper(StaticCrawler):
    """求人ボックス（食品製造の仕事）スクレイパー"""
    ...
    def parse(self, url: str) -> Generator[dict, None, None]:
        ...
        for pref_url in _PREF_URLS:
            page = 1
            self.logger.info("都道府県URL開始: %s", pref_url)

            while True:
                list_url = f"{pref_url}?pg={page}" if page > 1 else pref_url
                ...
                if page == 1 and not self.total_items:
                    ...
                    if not self.total_items:
                        self.total_items = 50000

                cards = soup.select("section.p-result_card")
                if not cards:
                    self.logger.info("%s pg%d: カードなし、終了", pref_url, page)
                    break
                ...
                self.logger.info("%s pg%d: %d件出力", pref_url, page, page_count)

                next_link = soup.select_one("a.c-pager_btn--next")
                if not next_link:
                    self.logger.info("%s pg%d: 次ページなし、終了", pref_url, page)
                    break
    ...
    def _parse_card(self, card, list_url: str) -> dict | None:
        ...
        title = _clean(
            ...
            or (a_tag.select_one("span.p-result_name").get_text(" ", strip=True) if a_tag.select_one("span.p-result_name") else a_tag.get_text(" ", strip=True))
        )
...
if __name__ == "__main__":
    ...
    scraper = KyujinBoxShokuhinSeizouScraper()
