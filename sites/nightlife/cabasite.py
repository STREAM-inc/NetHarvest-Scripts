"""
キャバサイト (cabasite.com) — 静岡・沼津エリアのキャバクラ求人スクレイパー

取得対象:
    - 店名 / カナ / 都道府県 / 住所 / TEL / 営業時間 / 定休日 / HP / 支払い方法(カード)
    - ジャンル(CAT_SITE) / エリア / 交通 / 給与 / 待遇
    - 料金システム: 時間制料金 / 飲み放題 / 延長 / お1人様の場合 /
      本指名料 / 場内指名料 / 同伴料 / キャストドリンク / TAX・サービス料

取得フロー:
    1. 求人一覧 search.php?run=true&type=job をページ送り (?q=0&page=N, page は 0 始まり)
    2. 一覧の各 li (a.disp_bl) から 店名/カナ/エリア+ジャンル と詳細 URL を取得
    3. 詳細ページ info.php?type=job&id=C... の dl / table.info-table から残りの項目を取得
    4. 1 件取得するごとに即 yield (Pattern B / 早期 yield)

ページネーション:
    - 1 ページ 20 件、page=0〜7 の計 8 ページ (≈160 件)
    - page>=8 は固定フォールバック 1 件 (C0000019) を返すため、
      「新規 ID が増えない」または「非満杯ページ かつ 新規 1 件以下」で停止する

実行方法:
    python scripts/sites/nightlife/cabasite.py
    docker compose exec worker python /app/bin/run_flow.py --site-id cabasite
"""
...
class CabasiteScraper(StaticCrawler):
    """キャバサイト (cabasite.com) 求人スクレイパー"""
    ...
    # 上記 EXTRA_COLUMNS のうち料金システム系の label 群。
    # _norm_label で正規化したキーで tbl / dl のどちらからでも拾う
    # (サイト側で表形式・定義リスト形式が混在しうるため)。
    _PRICE_LABELS = [
        "時間制料金", "飲み放題", "延長", "お1人様の場合",
        "本指名料", "場内指名料", "同伴料", "キャストドリンク", "TAX・サービス料",
    ]
    ...
    def _extract_listings(self, soup: BeautifulSoup, root_url: str) -> list[dict]:
        ...
        詳細リンク (a タグ) 自体が取れないブロックは、警告ログを出した上で
        continue で安全にスキップし、クローラー全体は停止させない。
        """
        listings: list[dict] = []
        # 各求人ブロック = 詳細ページへのリンクを持つ a.disp_bl
        for a in soup.select("a.disp_bl"):
            # 1. a タグの href 属性から詳細リンク文字列を抽出
            href = (a.get("href") or "").strip()

            # 4. 詳細リンク (a タグ / href) が取得できないブロックは警告して continue
            if not href:
            ...
            if not m:
                self.logger.warning("求人IDを含まない詳細リンクをスキップ: href=%s", href)
                continue

            # 2. 相対パスでも urljoin で root_url と安全に結合し https:// 始まりの絶対パスへ変換
    ...
    def _scrape_detail(self, listing: dict) -> dict | None:
        ...
        # 料金システム系 (時間制料金/飲み放題/延長/各種指名料/TAX 等) を
        # info-table 優先・dl フォールバックで補完する。
        for label in self._PRICE_LABELS:
    ...
    def _extract_tables(self, soup: BeautifulSoup) -> dict[str, str]:
        """info-table 系テーブルの th→td を辞書化 (給与/待遇/ご利用可能カード 等)。"""
        data: dict[str, str] = {}
