"""
luline 夜のお店選びドットコム（東北版） (luline.jp/tohoku) — ナイトワーク店舗スクレイパー

取得対象:
    - 店舗名 / 名称_カナ / 都道府県 / 住所 / TEL / 営業時間 / 定休日 / HP / サイト定義業種
    - エリア / 最寄り駅 / 平均年齢 / 各種料金 (指名・場内指名・同伴・サービス料・延長)
    - 求人情報 (table.arbeitMiddle): 業種 / 職種 / 資格 / 待遇 / 休日 / 勤務時間

勤務時間のカラム分割:
    求人の「時間」は 1 つの時刻だけでなく、曜日ごと (例: 平日 / 土日) や
    役割ごと (例: 【ホールスタッフ】17:00～翌2:30 / [ア] / [社]) に分けて
    書かれていることがある。これらは区分ごとに別カラムへ分割して取得する:
        勤務時間区分N (区分ラベル: 役割名や曜日) / 勤務時間N (時刻レンジ)
    単一の時刻しか無い場合は 勤務時間区分1 を空、勤務時間1 に時刻を入れる。

取得フロー (Pattern B: 一覧→詳細, 詳細1件ごとに即 yield):
    1. 一覧ページ (?page=N) から店舗ブロック (.listShopWrap) を収集
       - 一覧でしか取れない 名称_カナ / エリア / 平均年齢 をここで確保
       - luline.jp 内の詳細リンク (/tohoku/shop/view/N/) のみ採用 (tainew 等の外部リンクは除外)
    2. 各詳細ページ (div.accessInner table / table.arbeitMiddle) から
       住所/TEL/定休日/HP/最寄り駅/求人情報 等を取得
    3. 詳細を1件取得するごとに即 yield (途中中断でも無駄な通信をしない)

実行方法:
    python scripts/sites/nightlife/luline.py
    docker compose exec worker python /app/bin/run_flow.py --site-id luline
"""
...
# 詳細ページの主要情報テーブル (div.accessInner table) のラベル → 処理
_EXTRA_AREA = "エリア"
_EXTRA_STATION = "最寄り駅"
_EXTRA_AVG_AGE = "平均年齢"
...
# 求人テーブル (table.arbeitMiddle) のラベル → 処理
_EXTRA_JOB = "職種"
_EXTRA_QUALIFICATION = "資格"
_EXTRA_BENEFITS = "待遇"
...
class LulineScraper(StaticCrawler):
    """luline 夜のお店選びドットコム（東北版） スクレイパー"""
    ...
    EXTRA_COLUMNS: list[str] = [
        ...
        _EXTRA_FEE_EXTEND,
        _EXTRA_JOB,
        _EXTRA_QUALIFICATION,
        _EXTRA_BENEFITS,
        *_EXTRA_WORK_COLUMNS,
    ]

    def parse(self, url: str) -> Generator[dict, None, None]:
        # 一覧アイテム (一覧でしか取れないフィールドも保持) を全ページ巡回して収集
        listings = self._collect_listings(url)
        self.total_items = len(listings)
        self.logger.info("対象店舗数: %d", self.total_items)
        ...
        self.logger.info("取得完了: 候補%d件 取得%d件", self.total_items, saved)

    def _collect_listings(self, url: str) -> list[dict]:
        """全ページの一覧ブロックを収集。1ページ目は引数 url をそのまま使う。"""
        listings: list[dict] = []
        page = 1
        while True:
            ...
            if soup is None:
                break
            blocks = soup.select(".listShopWrap")
            if not blocks:
                break
            for block in blocks:
                parsed = self._parse_listing_block(url, block)
                if parsed:
    ...
    def _parse_detail(self, detail_url: str, soup: BeautifulSoup, listing: dict) -> dict | None:
        ...
        record = {
            ...
            _EXTRA_QUALIFICATION: self._join_lines(recruit.get("資格", "")),
            _EXTRA_BENEFITS: self._join_lines(recruit.get("待遇", "")),
        }
    ...
    def _recruit_to_dict(self, table) -> dict[str, str]:
        ...
        「時間」の曜日/役割分割や、「資格」「待遇」の箇条書きを後段で扱えるよう、
...
if __name__ == "__main__":
    ...
    scraper = LulineScraper()
    # 🔒 この URL は sites.yml に登録する url と完全一致させること (SSOT = sites.yml)
    scraper.execute("https://luline.jp/tohoku/shop_list/search/")
